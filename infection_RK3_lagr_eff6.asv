function [t,x,x_bind,x_endos] = infection_RK3_lagr_eff6
...
[k1,k1_bind,k1_endos]=inf_model(t0,x0,x0_bind,x0_endos);
...
while t<t_final
    ...
    % step control
    hmin = 16*eps*abs(t);
    hmax=min(hmax0,Dtau-sum_h);
    hmax=min(hmax,t_final-t+h);

    h=max(hmin,min(h,hmax));

    [k2, k2_bind, k2_endos]=inf_model(t+h/2,x+k1*h/2,x_bind+k1_bind*h/2,x_endos+k1_endos*h/2);
    [k3, k3_bind, k3_endos]=inf_model(t+3*h/4,x+3*k2*h/4,x_bind+3*k2_bind*h/4,x_endos+3*k2_endos*h/4);

    t_new=t+h;
    x_new=x+h*(2*k1+3*k2+4*k3)/9;
    x_bind_new=x_bind+h*(2*k1_bind+3*k2_bind+4*k3_bind)/9;
    x_endos_new=x_endos+h*(2*k1_endos+3*k2_endos+4*k3_endos)/9;

    [k4, k4_bind, k4_endos]=inf_model(t_new,x_new,x_bind_new,x_endos_new);

    % estimate error
    e = h*(-5*k1 + 6*k2 + 8*k3 - 9*k4)/72;
    e_bind=h*(-5*k1_bind + 6*k2_bind + 8*k3_bind - 9*k4_bind)/72;
    e_endos=h*(-5*k1_endos + 6*k2_endos + 8*k3_endos - 9*k4_endos)/72;

    err_est=max(max(norm(e./max(max(abs(x),abs(x_new)), threshold), inf),...
        norm(e_bind./max(max(abs(x_bind),abs(x_bind_new)), threshold), inf)),...
        norm(e_endos./max(max(abs(x_endos),abs(x_endos_new)), threshold), inf));
    ...
    if err_est <= rtol
        ...
        if length(tt)-i<10
            tt=[tt; zeros(1e4,1)];
            xx=[xx;zeros(1e4,length(x0))];
            xx_bind=[xx_bind;zeros(1e4,length(x0_bind))];
            xx_endos=[xx_endos;zeros(1e4,length(x0_bind))];
        end
...
%             x2=x;
            ...
            x_endos(endB1)=x_endos(endB1)+x_endos(endB1-1);
            x_endos(startB1+1:endB1-1)=x_endos(startB1:endB1-2);
            x_endos(startB1)=0;
            ...
            % reallocate endosomal virus in I2
            x_endos(endB2)=x_endos(endB2)+x_endos(endB2-1);
            x_endos(startB2+1:endB2-1)=x_endos(startB2:endB2-2);
            x_endos(startB2)=0;
            ...
            % reallocate co-infected cells and virus binded to them
                ...
                x_endos(ind_scB1Co(i2+n_bins*(i1-1)))=x_endos(ind_scB1Co(i2+n_bins*(i1-1)))+...
                    x_endos(ind_scB1Co(i2-1+n_bins*(i1-2)));
                x_endos(ind_scB2Co(i2+n_bins*(i1-1)))=x_endos(ind_scB2Co(i2+n_bins*(i1-1)))+...
                    x_endos(ind_scB2Co(i2-1+n_bins*(i1-2)));
             ...
             % translation of intermediate bins
                for i2=(n_bins-1):-1:(n_bins_inf+1)
                    for i1=i2:-1:(i2-age_end_inf/Dtau+1)                   
                        ...
                        x_endos(ind_scB1Co(i2+n_bins*(i1-1)))=x_endos(ind_scB1Co(i2-1+n_bins*(i1-2)));
                        x_endos(ind_scB2Co(i2+n_bins*(i1-1)))=x_endos(ind_scB2Co(i2-1+n_bins*(i1-2)));
                    end
                end
            ...
            % reset first bin
                ...
                x_endos(ind_scB1Co(i2+n_bins*(i1-1)))=0;
                x_endos(ind_scB2Co(i2+n_bins*(i1-1)))=0;
...
%             end
            ...
            [k1,k1_bind,k1_endos]=inf_model(t,x,x_bind,x_endos);
...
% u1=zeros(t_plot,t_plot);
% u2=u1;
% for k =n_bins:-1:1
%     if ~isempty(U1(~isnan(U1(:,k)),k))
%         u1(1:length(U1(~isnan(U1(:,k)),k)),k)=U1(~isnan(U1(:,k)),k);
%         u2(1:length(U2(~isnan(U2(:,k)),k)),k)=U2(~isnan(U2(:,k)),k);
%     end
% end
% bar3(U1)
title((t_plot))
...
% co=interp1(t,x(:,startCo:endCo),t_plot);
b1_co=interp1(t,x_bind(:,startB1Co:endB1Co)/2,t_plot);
b2_co=interp1(t,x_bind(:,startB2Co:endB2Co)/2,t_plot);
...
for i1=1:n_bins
    for i2=1:n_bins
        ...
        if index >0 
%             co_plot(i1,i2)=co(index);
            b1_co_plot(i1,i2)=b1_co_plot(i1,i2)+b1_co(index);
            b2_co_plot(i1,i2)=b2_co_plot(i1,i2)+b2_co(index);
        end
    end
end
figure
...
%% model
    function [dxdt,dxbind_dt,dxendos_dt] = inf_model(t,x,x_bind,x_endos)
    dxdt=x; % pre-allocate
    ...
    dxbind_dt(startB1)=dxbind_dt(startB1)+bind1; % binded virus added to binded virus balance of infected cells of first age bin
    ...
    for j = 1:n_bins_inf
        ...
        dxendos_dt(j)=Ein1-k_deathI*x_endos(j);
        dxendos_dt(startB2+j-1)=Ein2-k_deathI*x_endos(startB2+j-1);
    ...
    for k=1:length(Co)
        ...
        dxendos_dt(startB1Co-1+k)=Ein1-k_deathI*x_endos(startB1Co-1+k);
        dxendos_dt(startB2Co-1+k)=Ein2-k_deathI*x_endos(startB2Co-1+k);
