...
while t<t_final
    ...
    if 1.1*h >= t_final - t
        h = t_final - t;
    elseif 1.10*h+sum_h >= Dtau
        h = Dtau - sum_h;
        h1=abs(round(t+h)-(t+h));
        h=h+h1;
    end
...
b1_co=interp1(t,x_bind(:,startB1Co:endB1Co),t_plot);
b2_co=interp1(t,x_bind(:,startB2Co:endB2Co),t_plot);
...
for k =1:n_bins
    if ~isempty(U1(~isnan(U1(:,k)),k))
        u1(:,k)=U1(~isnan(U1(:,k)),k);
        u2(:,k)=U2(~isnan(U2(:,k)),k);
    end
end
inde=~isnan(U1);
...
function [dxdt,dxbind_dt] = inf_model(t,x,x_bind)
    ...
    % co-infected cells: death and virions binding
    for k=1:length(Co)
        ...
        dxbind_dt(startB1Co-1+k)=dxbind_dt(startB1Co-1+k)+Co_uptakeV1-...
                    (k_endos_in+k_deathI)*x_bind(startB1Co-1+k);

        dxbind_dt(startB2Co-1+k)=dxbind_dt(startB2Co-1+k)+Co_uptakeV2-...
                    (k_endos_in+k_deathI)*x_bind(startB2Co-1+k);
