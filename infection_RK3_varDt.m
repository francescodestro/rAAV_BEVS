function [t,x] = infection_RK3_varDt
...
% pre-allocate arrays
xx=zeros(10e3,length(x0)); % zeros(10e3,1.9e4);%
tt=zeros(10e3,1);
tt(1)=t0;
xx(1,1:length(x0))=x0;
i=2;
...
k1=inf_model(t0,xx(1,:));
r=norm(k1./max(abs(xx(1,:)),threshold), inf);
h = 0.8*rtol^(1/3)/r;
...
%         h=h*max(0.5, 0.8*(rtol/err_est)^(1/3));
%         h=5e-4;
    end
