function [t,x] = infection_upwind2
...
xx=zeros(n_steps+1,length(x0));
xx(1,:)=x0;
x=x0;

% first iteration
dxdt = inf_model(t(1),x0);
x=x+dxdt'*Dt;
xx(2,:)=x;
...
for i=2:n_steps
    ...
    r_death_co=(k_deathT*(0.5+...
        0.5*tanh((tau_L-t_inf)/0.3))+k_deathI*max(1,log(x(13)+x(14)+1e-16)).*(0.5+...
        0.5*tanh((t_inf-tau_L)/0.3)))*x(4);
    r_death_Igoi=(k_deathT*(0.5+...
        0.5*tanh((tau_L-t_inf)/0.3))+k_deathT*max(1,log(x(13)+1e-16)).*(0.5+...
        0.5*tanh((t_inf-tau_L)/0.3)))*x(5);
    r_death_Irc=(k_deathT*(0.5+...
        0.5*tanh((tau_L-t_inf)/0.3))+k_deathT*max(1,log(x(14)+1e-16)).*(0.5+...
        0.5*tanh((t_inf-tau_L)/0.3)))*x(6);
    ...
    dxdt(4)=k_bind*(x(5)*x(3)+x(2)*x(6))-k_deathI; % co-infected (#/mL)
    dxdt(5)=k_bind*x(1)*x(2)-k_deathI-k_bind*x(5)*x(3); % infected ITR/GOI (#/mL) 
    dxdt(6)=k_bind*x(1)*x(3)-k_deathI-k_bind*x(6)*x(2); % infected rep/cap (#/mL)
    ...
    n_L=(0.5+0.5*tanh((t_inf-tau_L)/0.3)).*(0.5+0.5*tanh((tau_repl_end-t_inf)/1)); % smooth transition
    n_VL=(0.5+0.5*tanh((t_inf-tau_VL)/1)).*(0.5+0.5*tanh((tau_VLend-t_inf)/10)); % smooth transition
    f_BVrepl=(1-(t_inf-tau_L)/(tau_repl_end+3-tau_L)).*n_L; % viral DNA replication activation function
...
%     x=x+dxdt*Dt; % first order upwind
    x=1/3*(2*Dt*dxdt'+4*x-xx(i-1,:)); % second order upwind
    xx(i+1,:)=x;
end
x=xx;
toc
...
%     k_degrBV=0; %1/h
    ...
    dxdt(2)=-k_bind*x(2)*(x(1)+x(5)+x(6)+x(4))-k_degrBV*x(2); % BV ITR/GOI (#/mL) 
    dxdt(3)=-k_bind*x(3)*(x(1)+x(5)+x(6)+x(4))-k_degrBV*x(3); % BV rep/cap (#/mL) 
    dxdt(4)=k_bind*(x(5)*x(3)+x(2)*x(6))-r_death_co; % co-infected (#/mL)
    dxdt(5)=k_bind*x(1)*x(2)-r_death_Igoi-k_bind*x(5)*x(3); % infected ITR/GOI (#/mL) 
    dxdt(6)=k_bind*x(1)*x(3)-r_death_Irc-k_bind*x(6)*x(2); % infected rep/cap (#/mL)
