...
% lumped transport to nucleus
K=1/(.5/k_endos_in+1/k_endos_out+1/k_nucl);

% uncertain parameters
k_AAVrel=0;
k_goi_repl=0; %.00001;
k_rep_syn=0;%1; % 1/h - fast step assumption
k_vp_syn=k_rep_syn;  % 1/h - fast step assumption

[t,x]=ode23(@inf_model,0:.1:20,x0);
toc
% plot(t,x(:,4))
% plot(t,sum(x(:,4),2))
% % figure
...
%     k_degrBV=0; %1/h
    ...
    dxdt(8)=k_bind*x(3)-K*x(8); % BV rep/cap on surface
...
%     dxdt(12)=k_endos_out*x(10)-k_nucl*x(12);
    ...
    % AAVempty_EC
    dxdt(21)=k_AAVrel*x(19)*C;

    % AAVfilled_EC
    dxdt(22)=k_AAVrel*x(20)*C;
